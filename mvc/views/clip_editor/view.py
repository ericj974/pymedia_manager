...
from mvc.views.clip_editor.action_params import FlipOrientation, RotationOrientation, ClipActionParams
...
class ClipEditorWindow(QMainWindow):
    ...
    def __init__(self, model: MainModel, controller: MainController, config=None, parent=None):
        ...
        if self._model.media_path:
            if self._model.media_path.suffix in FILE_EXTENSION_VIDEO:
    ...
    def on_media_path_changed(self, path: Path):
    ...
    def on_drop_media(self, event):
        for url in event.mimeData().urls():
            ...
            if path.is_file() and path.suffix in FILE_EXTENSION_VIDEO:
    ...
    def create_actions_shortcuts(self):
        ...
        self.exit_act = QAction(QIcon(str(icon_path / "exit.png")), 'Quit Editor', self)
        ...
        self.crop_act = QAction(QIcon(str(icon_path / "vid_cut.png")), "Crop", self)
        self.crop_act.setShortcut('C')
        self.crop_act.triggered.connect(self.media_widget.media_crop)

        self.resize_act = QAction(QIcon(str(icon_path / "resize.png")), "Resize", self)
        self.resize_act.setShortcut('Shift+Z')
        self.resize_act.triggered.connect(self.media_widget.media_zoom)

        self.rotate90_cw_act = QAction(QIcon(str(icon_path / "rotate90_cw.png")), 'Rotate 90º CW', self)
        self.rotate90_cw_act.setShortcut('R')
        self.rotate90_cw_act.triggered.connect(lambda: self.media_widget.media_rotate_90(RotationOrientation.cw))
        ...
        self.rotate90_ccw_act.triggered.connect(lambda: self.media_widget.media_rotate_90(RotationOrientation.ccw))

        self.flip_horizontal_act = QAction(QIcon(str(icon_path / "flip_horizontal.png")), 'Flip Horizontal',
                                           self)
        self.flip_horizontal_act.triggered.connect(lambda: self.media_widget.media_flip(FlipOrientation.horizontal))

        self.flip_vertical_act = QAction(QIcon(str(icon_path / "flip_vertical.png")), 'Flip Vertical', self)
        self.flip_vertical_act.triggered.connect(lambda: self.media_widget.media_flip(FlipOrientation.vertical))

        self.concat_act = QAction(QIcon(str(icon_path / "concat.png")), 'Concatenate', self)
        self.concat_act.triggered.connect(self.media_widget.media_concat)
    ...
    def create_editing_bar(self):
        ...
        self.lum_slider.valueChanged.connect(lambda: self.media_widget.media_set_lum_contrast(self.lum_slider.value(),
        ...
        self.contrast_slider.valueChanged.connect(
            lambda: self.media_widget.media_set_lum_contrast(self.lum_slider.value(),
                                                             self.contrast_slider.value()))
    ...
    def create_main_label(self, config):

        self.media_widget = ClipEditorWidget(self, config=config)
    ...
    def open_media(self, file: Path = None):
        ...
        if file is None:
            ...
            if file is not None:
                file = Path(file)
                try:
                    if file.suffix in FILE_EXTENSION_VIDEO:
        ...
        if file.suffix not in FILE_EXTENSION_VIDEO:
            self.media_widget.reset()
            self.setEnabled(False)
            return False
        ...
        if file:
            try:
                self.media_widget.open_media(file, fps_source='fps')
            except:
                self.media_widget.open_media(file, fps_source='tbr')
            self.cumul_scale_factor = 1
    ...
    def save_media(self, path: Path = None):
        """Save the image displayed in the label."""
        if path:
            if self.media_widget.clip_reader.clip:
                self.media_widget.save_media(path)
        else:
            if self.media_widget.clip_reader.clip:
                path = Path(QFileDialog.getSaveFileName(parent=self, caption="Save Media",
                                                        directory=str(self._model.dirpath),
                                                        filter="MP4 Files (*.mp4);;AVI Files (*.avi)")[0])
                if path.suffix == '':
