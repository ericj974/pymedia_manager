...
from common.comment import PersonEntity, ImageUserComment
...
class FaceEditorBatchWindow(QMainWindow):
    ...
    def detect_faces(self, files: list[Path]):
        self.results = []
        ...
        for i, result in enumerate(self.results):
            # Filename
            self.table_result.setItem(i, idx_col_filename, QTableWidgetItem(result.file.name))
    ...
    def save_to_img(self, index):
        if index.isValid():
            ...
            entity = PersonEntity(name=result.name, location=result.location)
            comment.add_entity(entity)
    ...
    def save_to_db(self, index):
        if index.isValid():
            ...
            for model in api.face_recognition_model:
                item_db = self._model_local.db.get_entry(name=result.name,
                ...
                # Representation
                embedding = api.face_encodings(imgs=[result.patch], recognition_model=model)[0]
    ...
    def on_return_pressed(self, index):
        if index.isValid():
            ...
            if cell.text() != cell.result.name:
                ...
                if cell.result.name != api.unknown_tag:
...
class MyQTableWidgetCell(QLineEdit):
    def __init__(self, result: DetectionResult, wordlist):
        super(MyQTableWidgetCell, self).__init__(result.name)
        self.result = result
        # Autocomplete
