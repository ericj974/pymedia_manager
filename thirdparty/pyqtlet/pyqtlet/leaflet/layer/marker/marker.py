...
class Marker(Layer):
    ...
    def setZIndexOffset(self, zoffset):
        js = '{layerName}.setZIndexOffset({zoffset})'.format(
            layerName=self._layerName, zoffset=zoffset)
        self.runJavaScript(js)

    def setIcon(self, icon: Icon):
        raise NotImplementedError
        js = '{layerName}.setIcon({iconName})'.format(
            layerName=self._layerName, iconName=icon.iconName)
        self.runJavaScript(js)
